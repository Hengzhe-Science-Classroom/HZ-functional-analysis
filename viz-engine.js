// === VizEngine: Visualization toolkit for functional analysis ===
class VizEngine {
    ...
    // Draw a curve from parametric function
    drawCurve(fn, tMin, tMax, steps, color, lw = 2) {
        const ctx = this.ctx;
    ...
    // Draw a filled region between two curves
    drawFilledRegion(fn1, fn2, tMin, tMax, steps, fill) {
        const ctx = this.ctx;
        const dt = (tMax - tMin) / steps;
        ctx.fillStyle = fill;
        ctx.beginPath();
